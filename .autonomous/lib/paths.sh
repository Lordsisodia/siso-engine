...
get_engine_path() {
    if [[ -n "${BB5_ENGINE_ROOT:-}" ]]; then
        ...
        # Default: 2-engine directory (parent of .autonomous)
        local script_dir
        script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        # script_dir is .../.autonomous/lib, so go up 2 levels
...
get_prompts_path() {
    local engine
    engine=$(get_engine_path)
    echo "$engine/.autonomous/prompts"
}
...
get_templates_path() {
    local engine
    engine=$(get_engine_path)
    echo "$engine/.autonomous/templates"
}
...
get_routes_path() {
    local engine
    engine=$(get_engine_path)
    echo "$engine/.autonomous/config/routes.yaml"
}
