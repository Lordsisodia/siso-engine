...
EXECUTOR_REPORT_DIR = resolver.get_analysis_path() / "executor-reports"
...
class VerifierAgent:
    ...
    def validate_threshold_fix(self, task_result: Dict[str, Any]) -> ValidationResult:
        ...
        file_path = PROJECT_DIR / "operations" / "skill-registry.yaml"
    ...
    def validate_task(self, task_result: Dict[str, Any]) -> ValidationResult:
        ...
        task_handlers = config.get("task_handlers", {})

        # Route to appropriate validator based on config or action_taken
        handler_config = task_handlers.get(task_id, {})
        handler_name = handler_config.get("handler", "")
        validation_rules = handler_config.get("validation_rules", [])

        # Route by handler name or fallback to keyword matching
        if handler_name == "threshold_fix" or "threshold" in task_result.get("action_taken", "").lower():
            return self.validate_threshold_fix(task_result)
        elif handler_name == "engine_path_fix" or "engine path" in task_result.get("action_taken", "").lower():
